var menus = {
    'kebaps': [
        {
            name: 'Adana Dürüm',
            price: '7.0',
            p_name: 'Adana Dürüm Porsiyon',
            p_price: '13.0',
            img: 'assets/img/sp11.jpg',
            order_no: 1
        },
        {
            name: 'Ciğer Dürüm',
            price: '8.0',
            p_name: 'Ciğer Porsiyon',
            p_price: '15.0',
            img: 'assets/img/sp12.jpg',
            order_no: 2
        },
        {
            name: 'Y.Ekmek Köfte',
            price: '6.0',
            p_name: 'Izgara Köfte Porsiyon',
            p_price: '12.0',
            img: 'assets/img/sp14.jpg',
            order_no: 4
        },
        {
            name: 'Kanat Porsiyon',
            price: '12.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp13.jpg',
            order_no: 5
        },
        {
            name: 'Çöp Şiş Dürüm',
            price: '8.0',
            p_name: 'Çöp Şiş Porsiyon',
            p_price: '16.0',
            img: 'assets/img/sp15.jpg',
            order_no: 6
        },
        {
            name: 'Tavuk Şiş Dürüm',
            price: '6.0',
            p_name: 'Tavuk Şiş Porsiyon',
            p_price: '12.0',
            img: 'assets/img/sp17.jpg',
            order_no: 7
        },
        {
            name: 'Karışık Kebap',
            price: '26.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp16.jpg',
            order_no: 8
        }
    ],
    'tantanas': [
        {
            name: 'Tantuni Dürüm',
            price: '7.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp1.jpg',
            order_no: 1
        },
        {
            name: 'Tantuni Yarım',
            price: '8.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp2.jpg',
            order_no: 2
        },
        {
            name: 'Ayvalık Tostu',
            price: '6.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp3.jpg',
            order_no: 3
        },
        {
            name: 'Çorba',
            price: '5.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp4.jpg',
            order_no: 4
        },
        {
            name: 'Patates Cipsi',
            price: '5.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp5.jpg',
            order_no: 5
        },
        {
            name: 'Salata',
            price: '4.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp6.jpg',
            order_no: 6
        },
        {
            name: 'Portakal Suyu',
            price: '5.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp7.jpg',
            order_no: 7
        },
        {
            name: 'Nar Suyu',
            price: '5.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp8.jpg',
            order_no: 8
        },
        {
            name: 'Susurluk Ayranı',
            price: '2.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp9.jpg',
            order_no: 9
        },
        {
            name: 'Şalgam',
            price: '2.0',
            p_name: '',
            p_price: '',
            img: 'assets/img/sp10.jpg',
            order_no: 10
        }
    ]
};
